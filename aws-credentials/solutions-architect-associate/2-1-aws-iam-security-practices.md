...
IAM 사용자는 AWS 계정의 ID로 IAM 사용자를 생성할 경우, 권한을 직접적으로 부여하지 않는 한 사용자는 계정 내에서 모든 서비스에 대한 액세스를 할 수 없어 IAM 그룹에 연결된 정책인 자격 증명 기반 정책을 생성하여 IAM 사용자에게 권한을 부여해야 한다.
...
정책에 의해 명시적으로 허용되지 않은 작업 또는 리소스는 기본적으로 거부되기 때문에 예를 들어 앞선 적용한 정책이 사용자에게 연결된 단일 정책이라면 이 사용자는 단일 서비스 작업을 수행할 수 있지만 다른 테이블에서는 작업을 수행할 수 없이에 여러 정책의 이해관계를 파악하고 조합하여 부여하는 것이 좋다.

## 2.3 정책 및 IAM 그룹

IAM 사용자들을 그룹으로 구성하고 그룹에 대한 정책을 연결할 수 있지만 이 경우 IAM 사용자들은 별도의 자격 증명을 갖고 있더라도 IAM 그룹내의 사용자들은 연결된 권한이 있기 때문에 그룹화를 사용하여 간편하게 권한을 제어할 수 있다.

<img src="https://docs.aws.amazon.com/images/IAM/latest/UserGuide/images/iam-intro-users-and-groups.diagram.png" />

IAM 사용자 또는 IAM 그룹에 여러 정책을 연결하여 다양한 권한을 부여할 수 있으며, 정책의 조합이 보안 주체의 유효 권한을 결정한다. 특정 작업과 리소스 모두에 대한 명시적으로 `Allow` 권한이 없는 위탁자에게는 이러한 권한이 부여되진 않는다.

## 2.4 페더레이션 사용자 세션 및 역할

페더레이션 보안 주체에게는 IAM 사용자처럼 AWS 계정에서 영구적인 ID가 부여되지 않는다. 권한을 부여하고자 한다면 역할이라는 엔터티를 생성하고 해당 역할의 권한을 정의할 수 있기 때문에 SAML 이나 OIDC 페더레이션 위탁자가 AWS에 로그인하면 사용자가 역할과 연결되고 정의된 권한이 부여된다.

## 2.5 자격 증명 기반 정책 및 리소스 기반 정책

자격 증명 기반 정책은 IAM 사용자, 그룹 또는 역할과 같은 IAM 자격 증명에 연결할 수 있는 권한 정책으로서 리소스 기반 정책은 Amazon S3 Bucket 또는 IAM Role 신뢰 정책과 같은 리소스에 연결하는 권한 정책이다.

* **자격 증명 기반 정책**
  AWS 계정에 속한 다수의 사용자, 그룹 및 역할에 독립적으로 연결할 수 있는 자격 증명 기반 정책이며 사용할 수 있는 관리형 정책은 다음과 같은 두 가지 유형이 있다.
  * **관리형 정책**
    * **AWS 관리형 정책**
      * AWS에서 생성 및 관리하는 하나의 관리형 정책으로 설계되어 정책 사용이 미숙하다면 AWS 관리형 정책 사용을 권장한다.
    * **고객 관리형 정책**
      * 사용자가 자신의 AWS 꼐정에서 생성 및 관리하는 관리형 정책이다. AWS 관리형 정책보다 정책에 대해 세부적으로 제어할 수 있어 JSON 정책 문서를 직접 생성해 IAM 정책을 생성, 편집, 검증을 진행할 수 있다.
  * **인라인 정책** 
    * 자신이 생성 및 관리하며, 단일 사용자, 그룹 또는 역할게 직접적으로 포함되는 정책이다. 대부분의 인라인 정책을 사용하지 않는 것을 권장한다.
* **리소스 기반 정책**
